...
/*
__device__ void exclusiveScan(int tidx, uint* output, int length) {
    // My note: since I know length is TPB, I won't bother to 
    // nextPow2(length). :)

    // Upsweep phase
    for(int twod = 1; twod < length; twod *= 2) {
        int twod1 = twod * 2;
        if( ((tidx & (twod1-1)) == 0) && (tidx < length) ) {
            output[tidx + twod1 - 1] += output[tidx + twod - 1];
        }
        __syncthreads();
    }
    ...
    for(int twod = length/2; twod >= 1; twod /= 2) {
        int twod1 = twod * 2;
        if( ((tidx & (twod1-1)) == 0) && (tidx < length) ) {
            int t = output[tidx + twod - 1];
            output[tidx + twod - 1] = output[tidx + twod1 - 1];
            output[tidx + twod1 - 1] += t;
        }
        __syncthreads();
    }
}
...
__global__ void kernelRenderCircles() {
    ...
    // to each thread
    uint privateCircleOrder[CIRCLES_PER_THREAD];
    ...
    // My note:
    // Summary each threads' private count to array circleCount
    circleCount[threadIndex] = privateCircleCount;
    //circleIndex[threadIndex] = privateCircleCount;
    __syncthreads();
    ...
    // My note
    // Phase2: render each pixel in this region
